...
### 2026-03-20 数学：定義問題フラッシュカードの原則削除
- **対象**: 数学全トピックのフラッシュカード
- **指摘**: 「〜とは何？」「〜とは？」形式で用語の定義を答えさせるフラッシュカードは中学数学になじみが薄い。一次式を選ぶ問題や、式の中の項を答える問題などの実践的な出題で代替すべき
- **対応**: 27ファイルから約75件の定義問題FCを削除。公式の想起（front が `$formula$` のもの、例: 分配法則 `$a(b+c) = ab + ac$`）は維持
- **汎用ルール**: 数学のフラッシュカードでは用語の定義を問う出題（「〜とは何？」→テキスト定義）を原則作成しない。代わりに「式の中の項を答えよ」「一次式を選べ」「計算結果は？」等の実践的な問題にする。公式・法則の想起（front が数式のもの）は例外として許容
- **削除判定基準**: (1) `back` に「とは」を含み (2) `front` がテキスト定義文（`$`で始まらない）→ 削除。`front` が数式（`$`で始まる）→ 維持
