...
| ②' **無料ティア専用** | **月 3,000円 / 日 300円** | コード（`aiCostCore.evaluateFreeGate`） | しうる |
| ③ ユーザー月次 | paid 350円＋段階デグレード / **free 600回** | コード | しうる |
| ④ ユーザー日次 | paid 月予算の1/10＋200回/日 / **free 40回** | コード | しうる |
| ⑤ 1リクエスト | 入力20,000トークン等 | コード | しうる |

**②〜⑤はコードなので実装バグで突破されうる。だから①が必須。** ①だけはコードから独立して効く。

### ⚠️ 2026-08-06 まで、②は `free` に効いていなかった

`aiChat` は `evaluateFreeGate` を通していたが、その中身が**無条件 allow**で、
`aiCostStats` への**計上だけして判定に使っていなかった**。無料側の歯止めは
「1人1日40回」だけで、理論上は **1人 ¥650/月**、上位1%（36人）が張り付けば
**¥23,000/月** まで伸びうる状態だった（実測は月 ¥560／3,638人＝1人 ¥0.15）。

2026-08-06 に②＋②'＋③（free 版）を通すよう修正。あわせて次を守っている:

- **集計が読めないとき `free` は止めない**（`paid` とは意図的に逆）。free は最安モデル
  固定で「1日40回」が先に効くため数分間の損失は数十円だが、deny に倒すと
  **3,000人の AI が一斉に沈黙**する。損失が非対称なので通す側に倒す。
- 判定に使う集計は **TTL 60秒キャッシュ**越し（追加 read は最大 1/ターン）。
- 個人の月次上限（600回）は費用ではなく**公平性**のため。1人が無料枠を
  食い潰して他の3,000人を締め出すのを防ぐ。実測の中央値は月数回なので通常は当たらない。
...
### 🚨 1-0b. 課金を切ると「AIの学習に使わせない」という**公開中の約束が嘘になる**

コスト対策として真っ先に思いつくのが「課金を外して無料枠に戻す」ことだが、
**Gemini はここが料金だけの問題ではない。**

| 枠 | 送信内容の扱い |
|---|---|
| **無料枠** | **Google が製品改善（＝モデルの学習）に利用する。人間によるレビューも入りうる** |
| **有料枠**（課金が有効） | 学習に利用しない |

つまり **課金の有無が、そのままデータの扱いを切り替える。**

そして次の2ページが「学習に使わせない」と**公開の場で断言している**:

- `pdf-workbook/lp/privacy.html` §4 …「AIモデルの学習（訓練）に利用しない設定で利用しており」
- `pdf-workbook/web/parents/index.html` §04 …「AIの学習には使わせていません」

**課金を外した瞬間、この記述はページを1文字も変えないまま虚偽になる。**
保護者に向けた約束なので、景表法・個人情報保護法の両面で問題になりうる。

> **原則: `gen-lang-client-0677055253` の課金は外さない。**
> 支出を抑えたいときは、課金を切るのではなく **予算アラート・APIクォータ・
> コード側の上限（②〜⑤）** で絞る。それが 1-1 以降の設定の目的でもある。
>
> どうしても無料枠に戻すなら、**先に上記2ページの文言を直してから**にする。
> 順番を逆にしない。

現状の確認（2026-08-02 実測: `billingEnabled: true`）:

```bash
gcloud billing projects describe gen-lang-client-0677055253 \
  --format="value(billingAccountName,billingEnabled)"
```

なお **OpenAI 側は課金に依存しない**。API に送ったデータは提供者のポリシーで
既定から訓練に使われず、加えて `llmOpenai.ts` が毎回 `store: false` を送って
30日保持もさせていない。依存関係があるのは Gemini だけ。
...
# 無料ティア専用キャップ（超過で無料ユーザーの AI だけ停止＋運営通知）
# 実測 月¥560 / 日¥18.5 に対し 5〜16倍の余裕。通常利用では当たらない。
AI_FREE_MONTHLY_CAP_JPY=3000
AI_FREE_DAILY_CAP_JPY=300

# 無料ユーザー1人あたりの月次呼び出し上限（公平性。1日40回×30日=1200 の抑制）
AI_FREE_USER_MONTHLY_CALL_CAP=600
...
## 5. 参考価格（確認日 2026-08-06）

出典:
- Gemini: https://ai.google.dev/gemini-api/docs/pricing （確認日 2026-07-25）
- OpenAI: https://openai.com/index/advancing-the-price-performance-frontier-with-gpt-5-6/ （確認日 2026-08-06）

| モデル | 入力 $/1M | キャッシュ入力 | 出力 $/1M | 用途 |
|---|---|---|---|---|
| `gemini-3.1-flash-lite` | 0.25 | — | 1.50 | `free` の既定 |
| **`gpt-5.6-luna`** | **0.20** | **0.02** | **1.20** | 有料の chat / classify / verify |
| **`gpt-5.6-terra`** | **2.00** | **0.20** | **12.00** | 有料の analysis / plan / counsel |
| `gpt-5.6-sol` | 5.00 | 0.50 | 30.00 | **使わない**（据え置き・常用で予算超過） |
| `gemini-3.5-flash-lite` | 0.30 | — | 2.50 | 予備 |
| `gemini-3.6-flash` | 1.50 | — | 7.50 | 予備 |

### 🔻 2026-07-30 の値下げ（`llmPrices.ts` に反映済み・2026-08-06）

| モデル | 変化 |
|---|---|
| `gpt-5.6-luna` | $1.00→**$0.20** / $6.00→**$1.20**（**−80%**） |
| `gpt-5.6-terra` | $2.50→**$2.00** / $15.00→**$12.00**（−20%） |
| `gpt-5.6-sol` | **据え置き**（2.5倍高速化のみ。値下げされていない） |

> ⚠️ 「Sol が80%安くなった」は誤り。安くなったのは **Luna**。Sol を常用すると
> 1ターン約 ¥21（入力24,000/出力600）＝月120ターンで ¥2,570 となり、月次予算 ¥350 の7倍。

### 用途別の実コスト（¥155/USD・入力24,000＋静的prefix 8,200 がキャッシュヒットする前提）

| 用途 | モデル | 1回 |
|---|---|---|
| chat（出力600） | luna | **¥0.63** |
| classify | luna | ¥0.02 |
| verify | luna | ¥0.10 |
| **analysis（出力1,000）** | **luna**（2026-08-06 に terra から変更） | **¥0.73**（旧 ¥7.01） |
| plan / counsel（出力1,200） | terra | **¥7.38** |

> **analysis を terra → luna に下げた理由**（2026-08-06）: analysis は「既に持っている
> 数字を読み上げる」用途で、生成後に `verify` で自己検証もしている
> （`aiPaidChat.verifyAnalysisAnswer`）。地力より検算が効く領域なので
> 「上位1回（¥7.01）」より「中位で生成＋中位で検証（¥0.73＋¥0.10）」が
> **8倍安く、質も落ちにくい**。質の劣化が見えたら
> `LLM_MODEL_ANALYSIS=openai:gpt-5.6-terra` で即戻せる。

> **Terra はちょうど Luna の10倍**（入力・出力とも）。**Terra 1回 ≒ 会話11ターン**なので、
> 原価率を決めるのは「Terra を月に何回呼ぶか」。月6回（週次分析＋プラン）に絞れば
> 1人あたり月 ¥147＝1,280円に対して **原価率 11.5%**。
>
> 💡 **「難しい判断＝上位モデル1回」より「Luna 2回（生成＋検証）」のほうが5.6倍安い**
> （¥1.26 vs ¥7.01）。`verify` の仕組みは `aiPaidChat.verifyAnalysisAnswer` に既にある。
