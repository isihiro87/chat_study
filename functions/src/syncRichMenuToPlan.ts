...
export const syncRichMenuToPlan = functions
  ...
  .https.onCall(
    async (
      data: SyncRichMenuToPlanInput,
      context
    ): Promise<SyncRichMenuToPlanOutput> => {
      // 1. 認可チェック（管理者メール限定）
      ...
      if (
        typeof plan !== 'string' ||
        !(VALID_PLANS as readonly string[]).includes(plan)
      ) {
        throw new functions.https.HttpsError(
          'invalid-argument',
      ...
      let planSource: 'paid' | 'trial' | 'trial_expired' | null = null;
      if (data.planSource !== undefined && data.planSource !== null) {
        if (
          data.planSource !== 'paid' &&
          data.planSource !== 'trial' &&
          data.planSource !== 'trial_expired'
        ) {
          throw new functions.https.HttpsError(
            'invalid-argument',
            'planSource は paid | trial | trial_expired のいずれかでなければなりません。'
          );
        }
      ...
      try {
        ...
        if (validPlan === 'premium') {
          ...
          update.planSource = planSource ?? null;
        }
