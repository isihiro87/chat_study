...
**問いの品質ルール（[2026-06-17追加] フラッシュカードは answer が一意に定まる形にする）:**
- **答えが複数になる問いは個数を明記する**。例: 「殻のない卵を水中にうむ脊椎動物のグループは？（2つ）」のように「（2つ）」「すべて答えよ」等を問いに入れる（front が「魚類・両生類」のように複数語のときが該当）。
- **答えが一意に定まる問いにする**。「どんな点で都合がよい？」「なぜ便利？」のような自由記述になりやすい問いは避け、答えとなる語が1つに決まる形に言い換える（例: front「乾燥」← back「殻は陸上で卵を主に何から守るのに役立つ？」）。一意化できないものは作らない。
- **問いに答えの決め手（定義的特徴）を列挙しない**（答えがバレる）。特徴は `explanation` 側に置き、問いは簡潔にする。悪い例「クジラは毛があり胎生で肺呼吸する。何類？」→ 良い例「クジラは何類に分類される？」。
