...
5. **答えが複数要素の問いは「組み合わせ」で問う**: [2026-06-17追加] 正解が複数のグループ等になる場合は「〜の組み合わせはどれですか」とし、選択肢も組み合わせ（例:「魚類・両生類」）にして正解が1つに定まるようにする。
6. **問題文に答えの決め手を書きすぎない**: [2026-06-17追加] 定義的特徴を問題文に列挙すると答えがバレる。特徴は `explanation` に置き、問題文は簡潔にする（悪い例「毛があり胎生で肺呼吸するクジラは何類？」→ 良い例「クジラは何類に分類されますか」）。
...
3. **選択肢の品質**
   ...
   - 紛らわしすぎて不公平でないか
   - 4つの選択肢が適切に分散しているか
   - **[2026-05-15追加] 形式・粒度・接尾辞が揃っているか**（メタ推理防止）
     - 接尾辞（「各国」「大陸」「時代」「文化」等）の有無を統一
     - 並列構造（「○○や○○」「A and B」）の有無を統一。正答だけ複数要素を含む形にしない
     - カテゴリの粒度を揃える（正答が地域名なら誤答も地域名、正答が人名なら誤答も人名）
     - 長さも極端に偏らせない（正答だけ突出して長い/短いを避ける）
...
- **選択肢・問題文のかっこ書きは原則不要**[2026-06-25追加]。理由「〜だから」・補足・例示「例: 〜」・全選択肢で共通する単位（円・本・km 等）のかっこは書かない。例外的に残すのは、正式略称の正式名称（`ASEAN（東南アジア諸国連合）`）・定着した同義語（`中央値（メジアン）`）・数式のグループ化・記号や図の説明・選択肢を区別する弁別情報（`副詞的用法（目的）`）のみ。とくに**理由のかっこ書きが正解選択肢だけに付くのは答えバレ**なので必ず削除する。
