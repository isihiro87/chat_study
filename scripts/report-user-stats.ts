...
async function main() {
  ...
  // 出題範囲設定（testScope.topics）。⚠️ funnel の liff_scope_save は web /scope
  // ページ経由だけを数えるため、トーク内 inline フロー（lastSource='line_inline'）を
  // 取りこぼし「範囲設定 約1割」と誤認していた。実態は user doc の testScope で数える。
  let withTestScope = 0;
  const scopeSourceCounts: Record<string, number> = {};
  ...
  for (const doc of targetDocs) {
    ...
    const ob =
      typeof u.onboardingState === 'string' ? u.onboardingState : '(unset)';
    inc(onboardingState, ob);
    // 実書き込みは "complete"（webhook / remindIncompleteOnboarding）。旧 "completed"
    // も念のため許容（過去ドキュメント互換）。以前は "completed" だけを数えていたため
    // 完了率が常に 0% と誤表示されていた（実データは state=complete が大半）。
    if (ob === 'complete' || ob === 'completed') onboardingCompleted++;
    ...
    const scope = u.testScope as
      ...
      | undefined;
    if (Array.isArray(scope?.topics) && scope!.topics.length > 0) {
      withTestScope++;
      const src =
        typeof scope?.lastSource === 'string' ? scope.lastSource : '(無印)';
      inc(scopeSourceCounts, src);
    }

    const plan = typeof u.plan === 'string' ? u.plan : '(unset)';
    inc(planCounts, plan);
    const planSource =
      typeof u.planSource === 'string' ? u.planSource : '(null)';
  ...
  try {
    ...
    console.warn(
      `[report-user-stats] answers scan failed: ${(e as Error).message}`
    );
  }
  ...
  console.log(
    `LINE 連携済み（lineUserId あり）        : ${withLineUserId}  (${pct(withLineUserId, total)})`
  );

  console.log('\n--- オンボーディング ---');
  console.log(
    `完了                                    : ${onboardingCompleted}  (${pct(onboardingCompleted, total)})`
  );
  for (const [k, v] of Object.entries(onboardingState).sort(
    (a, b) => b[1] - a[1]
  )) {
  ...
  console.log('\n--- 登録項目（プロフィール充足率） ---');
  console.log(
    `ニックネーム入力済                      : ${withNickname}  (${pct(withNickname, total)})`
  );
  console.log(
    `学年登録済                              : ${withGrade}  (${pct(withGrade, total)})`
  );
  for (const [k, v] of Object.entries(gradeCounts).sort()) {
    console.log(`  ${k.padEnd(6)} : ${v}`);
  }
  console.log(
    `教科登録済                              : ${withSubject}  (${pct(withSubject, total)})`
  );
  for (const [k, v] of Object.entries(subjectCounts).sort()) {
    console.log(`  ${k.padEnd(10)} : ${v}`);
  }
  console.log(
    `配信時刻登録済                          : ${withPreferredHour}  (${pct(withPreferredHour, total)})`
  );
  for (const [k, v] of Object.entries(hourCounts).sort(
    (a, b) => Number(a[0]) - Number(b[0])
  )) {
    console.log(`  ${k.padStart(2)}時 : ${v}`);
  }
  console.log(
    `出題範囲設定済（testScope）             : ${withTestScope}  (${pct(withTestScope, total)})`
  );
  console.log(
    `  ※ funnel の liff_scope_save は web /scope 経由のみ。実態はこの行を見る。`
  );
  for (const [k, v] of Object.entries(scopeSourceCounts).sort(
    (a, b) => b[1] - a[1]
  )) {
    console.log(`  経路 ${k.padEnd(12)} : ${v}  (${pct(v, withTestScope)})`);
  }
  ...
  console.log(
    `     └ paid 想定                              : ${premiumActive - trialNow}`
  );
  console.log(
    `  └ premium ラベルだが期限切れ              : ${premiumExpired}`
  );
  console.log(`\n  planSource 内訳:`);
  for (const [k, v] of Object.entries(planSourceCounts).sort(
    (a, b) => b[1] - a[1]
  )) {
  ...
  console.log('\n--- クイズ回答 ---');
  console.log(
    `1問でも回答したユーザー                 : ${answeredAny}  (${pct(answeredAny, total)})`
  );
  console.log(
    `  直近7日に回答                          : ${answeredLast7d}  (${pct(answeredLast7d, total)})`
  );
  console.log(
    `  直近30日に回答                         : ${answeredLast30d}  (${pct(answeredLast30d, total)})`
  );

  if (answersCount !== null) {
    console.log(`\nanswers コレクション件数（全回答）      : ${answersCount}`);
    console.log(
      `  １アカウント当たり平均回答数           : ${(answersCount / Math.max(answeredAny, 1)).toFixed(2)} 問 / 回答者`
    );
    console.log(
      `  全アカウント平均                       : ${(answersCount / Math.max(total, 1)).toFixed(2)} 問 / 登録者`
    );
  }
  ...
  if (usersWithStats > 0) {
    ...
    console.log(
      `  集計 totalAnswered                    : ${totalAnsweredFromStats}`
    );
    console.log(
      `  集計 totalCorrect                     : ${totalCorrectFromStats}`
    );
    console.log(
      `  正答率                                : ${pct(totalCorrectFromStats, totalAnsweredFromStats)}`
    );
  }
