 ...
 * 停止中はプロンプトが「毎日/週2回で届く」と案内することを禁じるため、
 * 配信頻度まわりのケースは期待値が変わる。
 */
...
const CASES: EvalCase[] = [
  ...
  },
  {
    ...
    // プロンプトが「『週2回（月・木）に届くよ』と案内しない」と明示的に
    // 禁じている。したがって停止中と平常時で期待値が逆になる。
    // 停止期間（JST 2026-08-01）を過ぎれば自動で平常時の期待値に戻る。
    // ⚠️ 停止中に「週2回」の語そのものを禁止すると厳しすぎる。実応答では
    //    「本来は週2回だけど今はおやすみ」と正しく枠組みを示すことがあり、
  ...
  {
    // 2026-08-04: 公開キャンペーン（8/11までに始めれば8/15まで無料）。
    // プロンプトに日数を直書きしていると、期限後も古い案内を続けてしまう。
    // ⚠️ キャンペーン終了後はこのケースを「3日間」を期待する形に直すこと。
    name: 'つづもん-体験の期間をキャンペーンどおりに言う',
    botKind: 'tsudumon',
    user: { grade: '中2', subject: 'history' },
    input: '無料でどれくらい使えるの？',
    expect: [/8月15日|8\/15/],
  ...
  {
    // 体験の入口は /start/ に変わった（2026-08-04）。
    name: 'つづもん-体験の入口をstartページで案内する',
    botKind: 'tsudumon',
    user: { grade: '中1', subject: 'history' },
    input: '無料体験ってどこから始めるの？',
    expect: [/tsudumon\.jp\/start/],
  },
  {
    // 2026-07-27: 体験中も「今日の1単元」を送るようにした。
    name: 'つづもん-体験中も今日の1単元が届く',
    botKind: 'tsudumon',
    user: { grade: '中2', subject: 'history' },
    input: 'お試し中でも毎日の単元って届くの？',
    forbid: [/届かない|届きません/],
  },
  {
