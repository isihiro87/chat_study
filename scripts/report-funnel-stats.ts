...
async function main() {
  ...
  // 2026-07 機能群: 「まだ習ってない」除外 / おためし1問
  const notLearnedTopicCount: Record<string, number> = {}; // applied の除外起点 topic -> 件数
  const notLearnedByMode: Record<string, number> = {}; // single / after -> 件数
  let sampleAnswered = 0;
  let sampleCorrect = 0;
  ...
  for (const doc of snap.docs) {
    ...
    // 「まだ習ってない」除外（全期間。topic 上位はコンテンツ改善の入力になる）
    if (t === 'not_learned_applied') {
      const topic = String(ev.context?.topic ?? '(unknown)');
      const mode = String(ev.context?.mode ?? '(unknown)');
      notLearnedTopicCount[topic] = (notLearnedTopicCount[topic] ?? 0) + 1;
      notLearnedByMode[mode] = (notLearnedByMode[mode] ?? 0) + 1;
    }

    // おためし1問（friend-add 直後の体験。正答率で難易度も確認）
    if (t === 'sample_question_answered') {
      sampleAnswered++;
      if (ev.context?.correct === true) sampleCorrect++;
    }
  }
  ...
  const types = Object.keys(totalByEvent).sort(
    (a, b) => totalByEvent[b] - totalByEvent[a]
  );
  if (types.length === 0) {
  ...
  try {
    ...
    if (ansSnap.size >= ANSWERS_SCAN_CAP) {
      console.log(
        `  ⚠️ answers が上限 ${ANSWERS_SCAN_CAP} に到達。一部のみ集計（実数はこれ以上）。`
      );
    }
  ...
  const totalExtraTaps = Object.values(extraTapBySrc7d).reduce(
    (s, n) => s + n,
    0
  );
  console.log('\n「もう一問解く / 追加で解く」タップ(7d):');
  console.log(`  合計 = ${totalExtraTaps} 回`);
  for (const [src, n] of Object.entries(extraTapBySrc7d).sort(
    (a, b) => b[1] - a[1]
  )) {
  ...
  for (const t of importantServerEvents) {
    ...
    if (total === 0) continue;
    console.log(
      `  ${t.padEnd(36)} total=${String(total).padStart(4)}  UU=${String(uu).padStart(3)}`
    );
  }

  // ========== 2026-07 機能群（まだ習ってない / 配信おやすみ / おためし1問） ==========
  console.log('\n========== 「まだ習ってない」出題除外 ==========');
  const nlTapUU = uuByEvent['not_learned_tap']?.size ?? 0;
  const nlAppliedUU = uuByEvent['not_learned_applied']?.size ?? 0;
  console.log(
    `  タップ UU=${nlTapUU} → 適用 UU=${nlAppliedUU}` +
      (nlTapUU ? `  (転換 ${pct(nlAppliedUU, nlTapUU)})` : '')
  );
  const modeKeys = Object.keys(notLearnedByMode).sort();
  for (const m of modeKeys) {
    console.log(`    mode=${m.padEnd(8)} ${notLearnedByMode[m]} 件`);
  }
  const topTopics = Object.entries(notLearnedTopicCount)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10);
  if (topTopics.length > 0) {
    console.log(
      '  除外起点 topic 上位（=「習ってない」指摘が集中する範囲。並び順・時期見直しの入力）:'
    );
    for (const [topic, n] of topTopics) {
      console.log(`    ${String(n).padStart(3)} 件  ${topic}`);
    }
  } else {
    console.log('    (まだ記録なし。2026-07-03 デプロイ後から蓄積)');
  }

  console.log('\n========== 配信おやすみ（ユーザー選択の一時停止） ==========');
  const pausedUU = uuByEvent['delivery_paused']?.size ?? 0;
  const resumedUU = uuByEvent['delivery_resumed']?.size ?? 0;
  console.log(
    `  おやすみ UU=${pausedUU} / 再開 UU=${resumedUU}` +
      (pausedUU ? `  (再開率 ${pct(resumedUU, pausedUU)})` : '')
  );
  console.log(
    '  ※ ブロックの受け皿。おやすみが増えてブロック率が下がるかを OA Manager と併読。'
  );

  console.log('\n========== おためし1問（友だち追加直後） ==========');
  console.log(
    `  回答 ${sampleAnswered} 件 / 正答率 ${pct(sampleCorrect, sampleAnswered)}` +
      `（UU=${uuByEvent['sample_question_answered']?.size ?? 0}）`
  );
  console.log(
    '  ※ 分母は同期間の新規 follow 数（report-user-stats 側）。回答率が低ければカード文言を見直す。'
  );

  console.log('\n========== Win-back 効果（retention 定点指標） ==========');
  const winbackUU = uuByEvent['winback_sent']?.size ?? 0;
  const restartTotalUU = new Set<string>();
  Object.values(restartBySource).forEach((s) =>
    s.forEach((u) => restartTotalUU.add(u))
  );
  const recoveryTotalUU = new Set<string>();
  Object.values(recoveryByFrom).forEach((s) =>
    s.forEach((u) => recoveryTotalUU.add(u))
  );
  ...
  if (srcKeys.length > 0) {
    for (const k of srcKeys) {
      console.log(
        `     └ source=${k.padEnd(14)} UU=${restartBySource[k].size}`
      );
    }
  } else {
  ...
  if (fromKeys.length > 0) {
    ...
    console.log(
      '     └ (まだ記録なし。日次 recalc が status_transition を記録する)'
    );
  }
  console.log(
